(function(){
  ...
  function badgeHtml(d,soon){
    if(d.state==='pending')return'<span class="predecision pending"><b>判定待ち</b></span>';
    if(d.state==='skip')return'<span class="predecision skip"><b>見送り</b></span>';
    return `<span class="predecision buy"><b>${soon?'買い・間近':'買い'}</b>${d.value?'<i class="value">期</i>':''}${d.v8?'<i class="v8">V</i>':''}</span>`;
  }
  function decorate(){
