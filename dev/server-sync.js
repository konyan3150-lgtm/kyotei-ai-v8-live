(function(){
  ...
  async function syncArchives(){
    ...
    archivesPromise=(async()=>{
      const indexRes=await fetch(`server-predictions-index.json?x=${Date.now()}`,{cache:'no-store'});if(!indexRes.ok){if(indexRes.status===404)return{imported:0,updated:0,total:0};throw Error('archive index HTTP '+indexRes.status)}
      ...
      let imported=0,updated=0;for(const item of index.archives||[]){const version=encodeURIComponent(item.updated_at||item.record_count||'1'),res=await fetch(`${item.file}?v=${version}`);if(!res.ok)continue;const archive=await res.json();if(archive?.schema!=='kyotei-v8-server-predictions-archive'||archive?.version!==1)continue;const merged=importRecords(archive.records);imported+=merged.imported;updated+=merged.updated}
